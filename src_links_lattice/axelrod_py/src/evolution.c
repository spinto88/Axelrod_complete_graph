	...
	for(step_n = 0; step_n < n_active_links; step_n++)
	{
	 	...
	 	if((random < h_ab) && (active_condition(mysys->agent[i], mysys->agent[j])))
		{
			...
			links_ones = 0;
                        ...
                        {
                                if(homophily(mysys->agent[i], mysys->agent[mysys->agent[i].neighbors[k]]) == 1.0/mysys->agent[i].f)
                                        links_ones++;
                                if(homophily(mysys->agent[i], mysys->agent[mysys->agent[i].neighbors[k]]) == 0.00)
                                        links_zero++;
                        }
                        ...
                        for(k = 0; k < mysys->agent[i].degree; k++)
                        {
                                if(homophily(mysys->agent[i], mysys->agent[mysys->agent[i].neighbors[k]]) == 1.0/mysys->agent[i].f)
                                {
                                        list_ones[lo] = mysys->agent[i].neighbors[k];
                                        lo++;
                                }
                                else if(homophily(mysys->agent[i], mysys->agent[mysys->agent[i].neighbors[k]]) == 0.00)
                                {
                                        list_zero[lz] = mysys->agent[i].neighbors[k];
                                        lz++;
                                }
                        }
			...
			fp = fopen("Links_created_destroyed","a");
        	        fprintf(fp, "%d,%d\n", mysys->links_created, mysys->links_destroyed);                       		    fclose(fp);
